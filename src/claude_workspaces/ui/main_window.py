...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        sb = self.statusBar()
        sb.setStyleSheet("QStatusBar { background: #2a2a2a; border-top: 1px solid #454545; }")
    ...
    def _install_ferramentas_minimize_btn(self) -> None:
        ...
        btn.setIcon(_ic("ph.minus", color="#c6c6c6"))
    ...
    def _build_terminal_pane(self) -> QWidget:
        ...
        tabs_qss = (
            ...
            "QTabBar { background: #2a2a2a; }"
            "QTabBar::tab { background: #2a2a2a; color: #8f8f8f; "
            "  padding: 6px 14px; border: 0; "
            "  border-right: 1px solid #454545; min-height: 22px; }"
            "QTabBar::tab:selected { background: #2e2e2e; color: #f4f4f4; "
            "  border-bottom: 2px solid #6fbf73; }"
            "QTabBar::tab:hover:!selected { color: #c6c6c6; }"
        )
        ...
        self._terminal_pane_minimize_btn.setIcon(
            ic("fa5s.window-minimize", color="#c6c6c6")
        )
        ...
        self._loading_corner.setStyleSheet(
            "color: #d8d8d8; font-size: 12px; padding: 0 8px;"
        )
        self._loading_corner.setVisible(False)
        ...
        def _build_pane(
            ...
            hl.addStretch(1)
            btn = _QPB()
            btn.setIcon(ic("fa5s.window-minimize", color="#c6c6c6"))
    ...
    def _build_console_runner_placeholder(self) -> QWidget:
        ...
        container.setStyleSheet(
            "#consoleRunnerPlaceholder { background: #2c3037; }"
        )
        ...
        body.setStyleSheet("background: #2c3037; color: #5f5f5f; padding: 28px;")
    ...
    def _toggle_pane_at(
        ...
        if sizes[idx] > 4:
            ...
            btn.setIcon(ic("fa5s.window-maximize", color="#c6c6c6"))
            btn.setIconSize(_QS(11, 11))
    ...
    def _add_section_header(self, label: str) -> None:
        ...
        item.setFont(0, font)
        item.setForeground(0, QBrush(QColor("#a2a2a2")))
    ...
    def _install_arquivos_bucket(self, ws_item: "QTreeWidgetItem", ws: Workspace) -> None:
        ...
        btn.setStyleSheet(
            "QPushButton { background: transparent; color: #c6c6c6; "
    ...
    def _refresh_empty_placeholder(self, ws_item: "QTreeWidgetItem") -> None:
        ...
        btn.setStyleSheet(
            "QPushButton { background: transparent; color: #8f8f8f; "
            "border: 1px dashed #4f4f4f; border-radius: 4px; "
            "padding: 4px 8px; margin: 0px; text-align: left; "
            "font-size: 11px; }"
            "QPushButton:hover { color: #f4f4f4; border-color: #5f5f5f; "
            "background: #383838; }"
        )
    ...
    def _focus_pane_from_sidebar(self, pane: str) -> None:
        ...
        if cache is None:
            cache = {
                "min": ic("fa5s.window-minimize", color="#c6c6c6"),
                "max": ic("fa5s.window-maximize", color="#c6c6c6"),
            }
    ...
    def _refresh_terminal_pane_title(self) -> None:
        ...
        if ws is None or term is None or not isinstance(term, TerminalWidget):
            placeholder = (
                "<span style='color:#5f5f5f'>Claude console — "
        ...
        console_html = (
            f"<span style='color:#f4f4f4;font-weight:600'>{display}</span>"
        )
            ...
            if tree_item is not None:
                ...
                if isinstance(child, TerminalChildWidget):
                    ...
                    if branch:
                        ...
                        if behind > 0:
                            sync_html += f" <span style='color:#d77657'>↓{behind}</span>"
                        mod_html = (
                            f" <span style='color:#d6b95c'>●{modified}</span>"
                            if modified > 0 else ""
                        )
                        branch_html = (
                            f" <span style='color:#5f5f5f'>·</span> "
                            f"<span style='color:#8f8f8f'>branch</span> "
                            f"<span style='color:#f4f4f4;font-weight:600'>"
                            f"⎇ {short}</span>"
                    ...
                    if model:
                        model_html = (
                            ...
                            f"<span style='color:#d8d8d8;font-weight:600'>"
                            f"{model}</span>"
                        )
                        model_full = model
        except Exception:
        ...
        _DOT_PREFIX = " <span style='color:#5f5f5f'>·</span> "
    ...
    def _update_reload_chip_style(self) -> None:
        ...
        if pending:
            ...
            self._reload_chip_btn.setStyleSheet(
                "QPushButton { background: rgba(61,138,95,0.18); color: #f4f4f4; "
